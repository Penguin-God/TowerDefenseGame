...
public class MultiBattleSkillDataCreater : MonoBehaviour, IMultiSkillDataCreater
{
    ...
    void SendSkillData()
    {
        ...
        GetComponent<PhotonView>().RPC(nameof(SetEnemyData), RpcTarget.OthersBuffered, main, client.GetSkillLevel(main), sub, client.GetSkillLevel(sub));
    }

    [PunRPC]
    void SetEnemyData(SkillType mainSkill, int mainLevel, SkillType subSkill, int subLevel)
    {
