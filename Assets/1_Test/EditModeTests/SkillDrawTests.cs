...
public class SkillDrawTests
{
    ...
    [TestCase(1, 5, 5, 3, 2)]
    [TestCase(5, 5, 5, 5, 0)]
    public void 더_이상_얻을_수_없는_스킬은_제외해야_함(int 태극Lv, int 흑결LV, int 마나LV, int 거학LV, int expected)
    {
        var inventory = CreateInventory(태극Lv, 흑결LV, 마나LV, 거학LV);
    ...
    {
        var inventroy = CreateInventory(태극Lv: 3, 흑결LV: 5, 마나LV:3, 거학LV:5);
        inventroy.AddSkill(new SkillAmountData(SkillType.태극스킬, mainAmount));
        inventroy.AddSkill(new SkillAmountData(SkillType.마나물약, subAmount));
    ...
    SkillDrawController CreateSut(SkillInventroy inventroy) => new SkillDrawController(AllSkills, new SkillDataGetter(SkillUpgradeDatas, inventroy, new SkillLevelData[] { }), SkillRewardData);
