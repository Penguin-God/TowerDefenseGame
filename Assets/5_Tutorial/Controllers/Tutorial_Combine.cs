...
public class Tutorial_Combine : TutorialController
{
    ...
    protected override void AddTutorials() // 조합 기능 만들면 그거로 설명하기
    {
        ...
        AddActionCommend(SetIndexNameToTarget);
        AddClickCommend("이제 조합 가능한 궁수 유닛이 보입니다. \n 클릭합시다", TARGET_UI_INDEX_NAME);
        AddClickCommend("조합 버튼을 클릭해 조합합니다.", "CombineButton 2");
        AddSpotLightActionCommend("기사 3개를 조합해 궁수를 조합했습니다!!", 
            () => Multi_UnitManager.Instance.FindUnit(0, UnitClass.archer).transform.position + new Vector3(0, 5, 0));
    }
    ...
    void SetIndexNameToTarget() => FindObjectsOfType<UI_UnitTracker>().First().name = TARGET_UI_INDEX_NAME;
}
